...
export class ShopManager {
	constructor(economy, prices, opts = {}) {
				...
				AssertNotNull(prices, "argument prices in " + this.constructor.name + ".js should not be null")
			
				AssertNotNull(opts, "argument opts in " + this.constructor.name + ".js should not be null")
	...
	priceOf(item) {
		return this.prices[item] ?? Infinity
	}

	canBuy(item) {
		return this.economy.canSpend(this.priceOf(item))
	}
	...
	_btnText(item) {
		return `${item} ($${this.priceOf(item)})`
	}

	_startPlacementFlow() {
		if (!this.canBuy('turret')) {
			Audio?.click?.(); return
		}
		Audio?.click?.()
		Mouse.onClick = (p) => {
			...
			if (!ok) {
				Audio?.click?.(); Mouse.onClick = null; return
			}
			if (this.buy('turret')) {
