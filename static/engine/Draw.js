export class Draw {
  ...
  splash(spawnPosition, targetLocation, angleWidth, length = 500) {
    ...
    const angle = Math.atan2(
      targetLocation.y - spawnPosition.y,
      targetLocation.x - spawnPosition.x
    );
  ...
  hpBar(position, currentHp, maxHp) {
    function toPercentage() {
      // returns a value between 0.0 and 1.0 representing percentage
  ...
  revertMouse(player, mousePosition) {
    ...
    const circleY =
      player.y + player.height / 2 + circleRadius * Math.sin(oppositeAngle);

    this.circle(circleX, circleY, playerRadius, "red");

    player.angle = Math.atan2(
      mousePosition.y - player.position.center.y,
      mousePosition.x - player.position.center.x
    );
  }

  objectThatIsMovingInRectangularPathAroundObject(
    camera,
    target,
    color = "red"
  ) {
  ...
  objectThatIsCirclingAroundObjectBasedOnMousePosition(player, mousePosition) {
