...
export class EnterVehicleExtension {
	...
	draw(ctx) {
		if (this.entered) {
			...
			Draw.text(ctx, this.vehicle.x, this.vehicle.y, 100, 40, 'F to exit', "12px Arial", "white")
		}
		else {
			...
			if (Distance.withinRadius(this.player, this.vehicle, 100)) {
				Draw.text(ctx, this.vehicle.x, this.vehicle.y, 100, 40, 'E to enter', "12px Arial", "white")
			}
		}
