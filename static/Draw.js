...
export class Draw {
	...
	static objectThatIsCirclingAroundObjectBasedOnMousePositionButBehind(ctx, player) {
		function getAngle(x1, y1, x2, y2) {
			return Math.atan2(y2 - y1, x2 - x1);
		}

		player.angle = 0
		const playerRadius = player.activeRadius;
	...
	static objectThatIsMovingInRectangularPathAroundObject(ctx, player, currentMousePosition) {
		...
		// Draw the circle
		Draw.circle(ctx, circleX, circleY, player.activeRadius, "red");
	...
	//this is not in use
	static fill(offscreenPalette, color) {
		const { canvas, ctx } = offscreenPalette;
		ctx.fillStyle = color
